const btnAgregaTarea = document.querySelector(".app__button--add-task");
const formAgregaTarea = document.querySelector(".app__form-add-task");
...
let tareas = JSON.parse(localStorage.getItem("tareas") || "[]"); let tareaSeleccionada = null;
...
btnAgregaTarea.addEventListener("click", function () {
    formAgregaTarea.classList.toggle("hidden");
});

formAgregaTarea.addEventListener("submit", function (evento) {
    ...
    const elementoTarea = crearElementoTarea(tarea1);
    ulTareas.appendChild(elementoTarea, elementoTarea);

    //JSON
    // stringify - Convertir un párametro a string
    ...
    formAgregaTarea.classList.add('hidden');
